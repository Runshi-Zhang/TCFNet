...
save_path = r'F:\zrs\project\pythonProject\get_hd\bone1'
import glob
...
for i in pathlist:
    if(i.__contains__('bone_predicted')):
        name = i.strip('bone_predicted.txt')
        if(int(name) < 10):
            loadmesh = glob.glob('E:/BaiduNetdiskDownload/STLmesh/face_bone_data/0' + name + '/bone/*.stl')
        else:
            loadmesh = glob.glob('E:/BaiduNetdiskDownload/STLmesh/face_bone_data/' + name + '/bone/*.stl')
        ...
        ms.generate_surface_reconstruction_screened_poisson(depth=9, samplespernode=2)#8,1.5
        ms.meshing_re_orient_faces_coherently()


        ms.compute_selection_by_condition_per_vertex(condselect='(q < 6)')#6
